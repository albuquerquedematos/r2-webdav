...
const SetTransferQueueContext = createContext<React.Dispatch<React.SetStateAction<TransferTask[]>>>(() => {})
...
export function TransferQueueProvider({ children }: { children: React.ReactNode }) {
	...
	function currentTaskUpdater(props: Partial<TransferTask>) {
		...
		return (tasks: TransferTask[]) => {
			...
			const newTasks = tasks.map((t) => (t === taskProcessing.current ? newTask : t))
			if (currentTask === taskProcessing.current) taskProcessing.current = newTask
			return newTasks
		}
	}

	useEffect(() => {
		const taskToProcess = transferTasks.find((task) => task.status === 'pending')
	...
	return (
		<TransferQueueContext.Provider value={transferTasks}>
			<SetTransferQueueContext.Provider value={setTransferTasks}>{children}</SetTransferQueueContext.Provider>
