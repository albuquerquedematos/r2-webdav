...
export async function fetchPath(path: string) {
	...
	const items: FileItem[] = Array.from(document.querySelectorAll('response'))
		.filter((response) => decodeURIComponent(response.querySelector('href')?.textContent ?? '').slice(WEBDAV_ENDPOINT.length) !== path.replace(/\/$/, ''))
		.map((response) => {
...
export async function multipartUpload(
	...
	const promises = parts.map((i) =>
		limit(async () => {
			...
			if (i === limit.concurrency) await new Promise((resolve) => setTimeout(resolve, 1000))
