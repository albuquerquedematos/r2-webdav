import { CircularProgress, Dialog, DialogContent, DialogTitle, List, ListItem, ListItemText, Tab, Tabs, Tooltip, Typography } from '@mui/material'
...
function ProgressDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
	...
	return (
		<Dialog open={open} onClose={onClose} fullWidth maxWidth='xs'>
			...
			<Tabs value={tab} onChange={(_, newTab) => setTab(newTab)} sx={{ '& .MuiTab-root': { flexBasis: '50%' } }}>
				<Tab label='Downloads' />
				<Tab label='Uploads' />
			</Tabs>
			{tasks.length === 0 ? (
					...
					<List>
						{tasks.map((task) => (
							<ListItem key={task.name}>
								<ListItemText primary={task.name} secondary={`${humanReadableSize(task.loaded)} / ${humanReadableSize(task.total)}`} />
								{task.status === 'failed' ? (
									...
									<CircularProgress variant='determinate' size={24} value={(task.loaded / task.total) * 100} />
								) : null}
							</ListItem>
						))}
					</List>
