	...
	return (
		<Drawer anchor='right' open={open} onClose={() => setOpen(false)}>
			<Box sx={{ width: 400, padding: 2, display: 'flex', flexDirection: 'column', height: '100%' }}>
				...
				<TextField label='File Name' value={noteName} onChange={(e) => setNoteName(e.target.value)} fullWidth sx={{ mb: 2 }} />

				<TextField label='Write your note...' multiline rows={15} variant='outlined' value={noteText} onChange={(e) => setNoteText(e.target.value)} fullWidth />

				<Button variant='contained' sx={{ mt: 2 }} onClick={handleSaveNote} disabled={!noteText.trim()}>
					Save & Upload Note
				</Button>
			</Box>
		</Drawer>
