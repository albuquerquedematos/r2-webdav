...
// Main Component
function Main({ search, onError }: { search: string; onError: (error: Error) => void }) {
	...
	return (
		<>
			...
			<TextPadDrawer open={showTextPadDrawer} setOpen={setShowTextPadDrawer} cwd={cwd} onUpload={fetchFiles} />
			...
			<MultiSelectToolbar
				...
				onDelete={async () => {
					...
					for (const key of multiSelected) await fetch(`/webdav/${encodeKey(key)}`, { method: 'DELETE' })
					fetchFiles()
				}}
				onShare={() => {
					if (multiSelected?.length !== 1) return
