...
export class S3Client {
	...
	public async s3_fetch(input: string, init?: RequestInit) {
		...
		const canonicalQueryString = [...url.searchParams].map(([key, value]) => encodeURIComponent(key) + '=' + encodeURIComponent(value)).join('&')
		...
		const signedHeaderKeys = [...headers.keys()].filter((header) => header === 'host' || header === 'content-type' || header.startsWith('x-amz-'))
		const canonicalHeaders = signedHeaderKeys.map((key) => `${key}:${headers.get(key)}\n`).join('')
		...
		const canonicalRequest = [method, canonicalUri, canonicalQueryString, canonicalHeaders, signedHeaders, hashedPayload].join('\n')

		const hashedRequest = arrayBufferToHex(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(canonicalRequest)))
		...
		const dateKey = await hmacSHA256(new TextEncoder().encode('AWS4' + this.secretAccessKey), datetime.slice(0, 8))
