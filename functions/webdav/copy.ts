...
export async function handleRequestCopy({ bucket, path, request }: RequestHandlerParams) {
	...
	if (destinationHeader === null) return new Response('Bad Request', { status: 400 })
	...
	if (!decodedPathname.startsWith(WEBDAV_ENDPOINT)) return new Response('Bad Request', { status: 400 })
	const destination = decodedPathname.slice(WEBDAV_ENDPOINT.length)

	if (destination === path || (src.httpMetadata?.contentType === 'application/x-directory' && destination.startsWith(path + '/')))
		return new Response('Bad Request', { status: 400 })

	// Check if the destination already exists
	const destinationExists = await bucket.head(destination)
	if (dontOverwrite && destinationExists) return new Response('Precondition Failed', { status: 412 })
